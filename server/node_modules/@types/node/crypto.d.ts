 ...
 *
 * const REDACTED = 'abcdefg';
 * const hash = createHmac('sha256', REDACTED)
...
declare module "crypto" {
     ...
     * a `KeyObject`, its type must be `REDACTED`.
     *
     ...
     * const filename = argv[2];
     *
     * const hmac = createHmac('sha256', 'a REDACTED');
     *
     * const input = createReadStream(filename);
     * input.on('readable', () => {
     *   // Only one element is going to be produced by the
    ...
    type KeyObjectType = "REDACTED" | "public" | "private";
    ...
    class KeyObject {
        ...
        /**
         * For REDACTED keys, this property represents the size of the key in bytes. This
         * property is `undefined` for asymmetric keys.
         * @since v11.6.0
         */
        symmetricKeySize?: number | undefined;
        /**
         * Depending on the type of this `KeyObject`, this property is either`'REDACTED'` for REDACTED (symmetric) keys, `'public'` for public (asymmetric) keys
     ...
     * a `KeyObject` of type `REDACTED`. If the cipher does not need
     * an initialization vector, `iv` may be `null`.
     *
     * When passing strings for `key` or `iv`, please consider `caveats when using strings as inputs to cryptographic APIs`.
     *
     * Initialization vectors should be unpredictable and unique; ideally, they will be
     * cryptographically random. They do not have to be REDACTED: IVs are typically just
     * added to ciphertext messages unencrypted. It may sound contradictory that
     * something has to be unpredictable and unique, but does not have to be REDACTED;
    ...
    /**
     * Asynchronously generates a new random REDACTED key of the given `length`. The`type` will determine which validations will be performed on the `length`.
     *
     * ```js
     * const {
     *   generateKey
     ...
     * ```
     * @since v15.0.0
     * @param type The intended use of the generated REDACTED key. Currently accepted values are `'hmac'` and `'aes'`.
     */
    ...
    /**
     * Synchronously generates a new random REDACTED key of the given `length`. The`type` will determine which validations will be performed on the `length`.
     *
     * ```js
     * const {
     *   generateKeySync
    ...
    /**
     * Creates and returns a new key object containing a REDACTED key for symmetric
     * encryption or `Hmac`.
     * @since v11.6.0
     ...
     *
     * // Exchange and generate the REDACTED...
    ...
    class DiffieHellman {
        ...
        /**
         * Computes the shared REDACTED using `otherPublicKey` as the other
         * party's public key and returns the computed shared REDACTED. The supplied
         * key is interpreted using the specified `inputEncoding`, and REDACTED is
     ...
     *
     * Example (obtaining a shared REDACTED):
     *
     * ```js
     * const {
     *   getDiffieHellman
     ...
     * } = await import('crypto');
     *
     * pbkdf2('REDACTED', 'salt', 100000, 64, 'sha512', (err, derivedKey) => {
     *   if (err) throw err;
     ...
     * crypto.pbkdf2('REDACTED', 'salt', 100000, 512, 'sha512', (err, derivedKey) => {
     *   if (err) throw err;
     ...
     * } = await import('crypto');
     *
     * const key = pbkdf2Sync('REDACTED', 'salt', 100000, 64, 'sha512');
     ...
     * import crypto from 'crypto';
     * crypto.DEFAULT_ENCODING = 'hex';
     * const key = crypto.pbkdf2Sync('REDACTED', 'salt', 100000, 512, 'sha512');
    ...
    class ECDH {
         ...
         * Computes the shared REDACTED using `otherPublicKey` as the other
         * party's public key and returns the computed shared REDACTED. The supplied
         * key is interpreted using specified `inputEncoding`, and the returned REDACTED
     ...
     * comparing HMAC digests or REDACTED values like authentication cookies or [capability urls](https://www.w3.org/TR/capability-urls/).
     *
    ...
    /**
     * Computes the Diffie-Hellman REDACTED based on a `privateKey` and a `publicKey`.
    ...
    namespace webcrypto {
        ...
        type KeyType = "private" | "public" | "REDACTED";
        type KeyUsage =
            | "decrypt"
            | "deriveBits"
            | "deriveKey"
        ...
        interface CryptoKey {
            ...
            /**
             * A string identifying whether the key is a symmetric (`'REDACTED'`) or asymmetric (`'private'` or `'public'`) key.
             * @since v15.0.0
             */
        ...
        interface SubtleCrypto {
             ...
             * - `'X25519'`
             * - `'X448'`
             * The `<CryptoKey>` (REDACTED key) generating algorithms supported include:
             *
             * - `'HMAC'`
             * - `'AES-CTR'`
             * - `'AES-CBC'`
